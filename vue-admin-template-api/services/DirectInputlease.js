...
// 查询任务列表
function queryDirectInputleaseList(req, res, next) {
    ...
    if (!err.isEmpty()) {
        ...
        let query = `select * from directInputlease where projectId = ${projectID}`;
        querySql(query)
            .then(data => {
...
async function addDirectInputleaseDetail(req, res, next) {
    ...
    if (!err.isEmpty()) {
        ...
        for (let i = 0; i < tableDate.length; i++) {
            count++
            sql = `insert into directInputlease(projectId, equipmentNumber, equipmentName, expenseType, monthlyDepreciation, workingHours, developmentHours) values(${projectID}, '${tableDate[i].equipmentNumber}', '${tableDate[i].equipmentName}', '${tableDate[i].expenseType}','${tableDate[i].monthlyDepreciation}', '${tableDate[i].workingHours}','${tableDate[i].developmentHours}')`;
            await querySql(sql)
                .then(data => {
                    succCount++
                }).catch(err => {
...
function deleteDirectInputleaseDetail(req, res, next) {
    const err = validationResult(req);
    if (!err.isEmpty()) {
        const [{ msg }] = err.errors;
        next(boom.badRequest(msg));
        ...
        const query = `delete from directInputlease where id='${id}'`;
        querySql(query)
            .then(data => {
                if (!data || data.length === 0) {
                    res.json({
