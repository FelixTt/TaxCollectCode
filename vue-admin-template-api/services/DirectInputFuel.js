...
// 查询任务列表
function queryDirectInputFuelList(req, res, next) {
    ...
    if (!err.isEmpty()) {
        ...
        let query = `select * from directInputFuel where projectId = ${projectID}`;
        querySql(query)
            .then(data => {
...
function deleteDirectInputFuelDetail(req, res, next) {
    const err = validationResult(req);
    if (!err.isEmpty()) {
        const [{ msg }] = err.errors;
        next(boom.badRequest(msg));
        ...
        const query = `delete from directInputFuel where id='${id}'`;
        querySql(query)
            .then(data => {
                if (!data || data.length === 0) {
                    res.json({
