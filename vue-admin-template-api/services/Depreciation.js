...
// 查询任务列表
function queryDepreciationList(req, res, next) {
    ...
    if (!err.isEmpty()) {
        ...
        let query = `select * from depreciation where projectId = ${projectID}`;
        querySql(query)
            .then(data => {
...
// 添加人工明细信息
async function addDepreciationDetail(req, res, next) {
    const err = validationResult(req);
    if (!err.isEmpty()) {
        const [{ msg }] = err.errors;
        next(boom.badRequest(msg));
...
function deleteDepreciationDetail(req, res, next) {
    const err = validationResult(req);
    if (!err.isEmpty()) {
        const [{ msg }] = err.errors;
        next(boom.badRequest(msg));
        ...
        const query = `delete from depreciation where id='${id}'`;
        querySql(query)
            .then(data => {
                if (!data || data.length === 0) {
                    res.json({
