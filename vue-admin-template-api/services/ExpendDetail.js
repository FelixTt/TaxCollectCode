...
// 查询任务列表
function queryExpendDetailList(req, res, next) {
...
// 添加人工明细信息
async function addExpendDetailDetail(req, res, next) {
    const err = validationResult(req);
    if (!err.isEmpty()) {
        const [{ msg }] = err.errors;
        next(boom.badRequest(msg));
...
function deleteExpendDetailDetail(req, res, next) {
    const err = validationResult(req);
    if (!err.isEmpty()) {
        const [{ msg }] = err.errors;
        next(boom.badRequest(msg));
        ...
        const query = `delete from ExpendDetail where id='${id}'`;
        querySql(query)
            .then(data => {
                if (!data || data.length === 0) {
                    res.json({
