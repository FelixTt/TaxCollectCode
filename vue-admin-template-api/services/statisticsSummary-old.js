...
async function updateStatisticsSummary(req, res, next) {
    ...
    if (!err.isEmpty()) {
        ...
        if (exitFlag == null || exitFlag.length === 0) {
            ...
            // 新增
            // console.log("add")
            let { Jan, Feb, Mar, Apr, May, Jun, Jul, Aug, Sep, Oct, Nov, Dec, year, yearSum } = MonthInfo
            let query = `insert into StatisticsSummary(projectId, JanSum, FebSum, MarSum, AprSum, MaySum, JunSum, JulSum, AugSum, SepSum, OctSum, NovSum, DecSum, year, yearSum) values(${projectID}, ${Jan}, ${Feb}, ${Mar}, ${Apr}, ${May}, ${Jun}, ${Jul}, ${Aug}, ${Sep}, ${Oct}, ${Nov}, ${Dec}, '${year}', ${yearSum})`
            querySql(query)
            .then(data => {
                res.json({
                    code: CODE_SUCCESS,
            ...
            console.log("工资not first")
            // 修改
            let { Jan, Feb, Mar, Apr, May, Jun, Jul, Aug, Sep, Oct, Nov, Dec, year, yearSum } = MonthInfo
            // console.log("edit")
            let query = `update StatisticsSummary set JanSum=${Jan}, FebSum=${Feb}, MarSum=${Mar}, AprSum=${Apr}, MaySum= ${May}, JunSum=${Jun}, JulSum=${Jul}, AugSum=${Aug}, SepSum=${Sep}, OctSum=${Oct}, NovSum=${Nov}, DecSum=${Dec}, yearSum=${yearSum}, year='${year}' where projectId=${projectID} and year = '${year}'`
            querySql(query)
                .then(data => {
                    res.json({
                        code: CODE_SUCCESS,
...
async function updateInsurancesAndFundSum(req, res, next) {
    ...
    if (!err.isEmpty()) {
        ...
        let exitFlag = await querySql(query)
        if (exitFlag == null || exitFlag.length === 0) {
            console.log("五险一金 first")
            // 新增
            // console.log("add")
            let { JanInsAndFund, FebInsAndFund, MarInsAndFund, AprInsAndFund, MayInsAndFund, JunInsAndFund, JulInsAndFund, AugInsAndFund, SepInsAndFund, OctInsAndFund, NovInsAndFund, DecInsAndFund, yearInsAndFundSum, year } = MonthInfo
...
async function updateDirectInputMaterial(req, res, next) {
    ...
    if (!err.isEmpty()) {
        ...
        let { MonthInfo, projectID } = req.body;
        let { year } = MonthInfo
        let query = `select * from StatisticsSummaryDirectInput where projectId = ${projectID} and year = '${year}'`;
        let exitFlag = await querySql(query)
        if (exitFlag == null || exitFlag.length === 0) {
            // 新增
            // console.log("add")
