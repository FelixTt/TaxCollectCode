...
async function addDirectInputMaterialDetail(req, res, next) {
    ...
    if (!err.isEmpty()) {
        ...
        for (let i = 0; i < tableDate.length; i++) {
            count++
            sql = `insert into directInputMaterial(projectId, year, month, materialsName, unit, quantity, price) values(${projectID},'${tableDate[i].year}','${tableDate[i].month}', '${tableDate[i].materialsName}', '${tableDate[i].unit}', '${tableDate[i].quantity}','${tableDate[i].price}')`;
            await querySql(sql)
                .then(data => {
                    succCount++
                }).catch(err => {
