        ...
        <el-table-column prop="specialIncome" label="特殊收入">
          <template slot-scope="scope">
            <el-input
              ...
              v-model="singleTableParams.specialIncome"
            >
            </el-input>
            <span v-else>
              {{ scope.row.specialIncome }}
            </span>
          </template>
        </el-table-column>
        <el-table-column prop="materialCost" label="研发活动直接形成产品对应材料费用">
          <template slot-scope="scope">
            <el-input
              v-if="scope.row.isEdit == true"
              v-model="singleTableParams.materialCost"
            >
            </el-input>
            <span v-else>
...
export default {
  ...
  methods: {
    ...
    handleSuccess({ results, header }) {
      ...
      // 1、header必须一样
      // 2、不能为空
      console.log("heaaaaa", header)
      let checkArr = ['年份', '月份', '特殊收入', '研发活动直接形成产品对应材料费用']
