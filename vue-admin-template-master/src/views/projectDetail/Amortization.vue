      ...
      >
        ...
        <el-table-column prop="IntangibleAssetsNum" label="无形资产编号">
          <template slot-scope="scope">
            <el-input
              v-if="scope.row.isEdit == true"
              v-model="singleTableParams.IntangibleAssetsNum"
            >
            </el-input>
            <span v-else>
              {{ scope.row.IntangibleAssetsNum }}
            </span>
          </template>
        </el-table-column>
        <el-table-column prop="IntangibleAssetsType" label="无形资产类型">
          <template slot-scope="scope">
            <el-input
              v-if="scope.row.isEdit == true"
              v-model="singleTableParams.IntangibleAssetsType"
            >
            </el-input>
            <span v-else>
              {{ scope.row.IntangibleAssetsType }}
            </span>
          </template>
        </el-table-column>
        <el-table-column prop="IntangibleAssetsName" label="无形资产名称">
          <template slot-scope="scope">
            <el-input
              v-if="scope.row.isEdit == true"
              v-model="singleTableParams.IntangibleAssetsName"
            >
            </el-input>
            <span v-else>
              {{ scope.row.IntangibleAssetsName }}
            </span>
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    handleSuccess({ results, header }) {
      ...
      // 1、header必须一样
      // 2、不能为空
      let checkArr = ['年份', '月份', '研发项目序号', '种类', '编号', '摘要', '无形资产编号', '无形资产类型', '无形资产名称', '费用类型', '月折旧额 （元）', '工作工时', '研发工时']
    ...
    async save() {
      ...
      for(let i=0; i<ocpTmpArr.length; i++) {
        let rate = 0
        if(parseFloat(ocpTmpArr[i].workTime) != NaN && parseFloat(ocpTmpArr[i].workTime) != 0) {
          rate = parseFloat(ocpTmpArr[i].developTime) / parseFloat(ocpTmpArr[i].workTime)
        }
