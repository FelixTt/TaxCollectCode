      ...
      >
        ...
        <el-table-column prop="domesticCompCost" label="委托境内机构或个人进行研发活动所发生的费用">
          <template slot-scope="scope">
            <el-input
              v-if="scope.row.isEdit == true"
              v-model="singleTableParams.domesticCompCost"
            >
            </el-input>
            <span v-else>
              {{ scope.row.domesticCompCost }}
            </span>
          </template>
        </el-table-column>
        <el-table-column prop="abroadCompCost" label="委托境外机构进行研发活动所发生的费用">
          <template slot-scope="scope">
            <el-input
              v-if="scope.row.isEdit == true"
              v-model="singleTableParams.abroadCompCost"
            >
            </el-input>
            <span v-else>
              {{ scope.row.abroadCompCost }}
            </span>
          </template>
        </el-table-column>
        <el-table-column prop="abroadPerCost" label="委托境外个人进行研发活动所发生的费用">
          <template slot-scope="scope">
            <el-input
              v-if="scope.row.isEdit == true"
              v-model="singleTableParams.abroadPerCost"
            >
            </el-input>
            <span v-else>
              {{ scope.row.abroadPerCost }}
            </span>
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    handleSuccess({ results, header }) {
      ...
      // 1、header必须一样
      // 2、不能为空
      let checkArr = ['年份', '月份', '日期', '研发项目序号', '种类', '编号', '摘要', '委托境内机构或个人进行研发活动所发生的费用', '委托境外机构进行研发活动所发生的费用', '委托境外个人进行研发活动所发生的费用']
